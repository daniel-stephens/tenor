...
def main():
    ...
    doc_dir = '/srv/www/active-topic-modeling/cleaned_up_code/camera_ready_copy/flask_app/Data/community_resilience/community_resilience_clean.json'
    processed_doc_dir = '/srv/www/active-topic-modeling/cleaned_up_code/camera_ready_copy/flask_app/Data/community_resilience/community_resilience_processed.pkl'
    ...
    if args.model_type == 'ETM':
        ...
        if args.model_path == 'none':
            ...
            model = Topic_Model(args.num_topics, 0, args.model_type, processed_doc_dir, len(df), {}, True, './Model/{}_{}.pkl'.format(args.model_type, args.num_topics))
        else:
            model = Topic_Model(args.num_topics, 0, args.model_type, processed_doc_dir, len(df), {}, True, args.model_path)
    ...
    if args.print_docs == True:
        ...
        for topic, docs in topic_document_probas.items():
            for idx in range(0, 5):
                doc_id = docs[idx][0]
                topic_distibution, topic_res_num = model.predict_doc_with_probs(int(doc_id), topics)
                print('*'*30)
                print('keywords:', topic_res_num[0][1])
                print('Label: ', labels[doc_id])
